...
public class MessageWallActivity extends ListActivity {
	...
	public void onCreate(Bundle savedInstanceState) {
	    ...
	    //Get comment and timestamp info from backend
		    ...
		    //table row list in raw JSON string (for format example: refer JSON REST API)
		    	try{
		    		...
		    		if(status == 101){
		    			...
		    			mComments = new String[length];
		    			mTimestamp = new String[length];
		    			
		    			for(int i = 0; i < length; i++){
		    				
		    				mComments[length - i - 1] = array.getJSONObject(i).getString("comment"); 
		    				mTimestamp[length - i - 1] = array.getJSONObject(i).getString("timestamp");
		    				
		    			}
		    			
		    			
		    			//Add received information to listview
